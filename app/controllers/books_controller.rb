  ...
  def create
    ...
    lesson.aasm_book do
      Book.transaction do
        ...
        return render json: { status: :no_lesson_found, message: 'すでに他の時間帯でレッスンを予約済みです' } if Book.over_booking(@book, current_user.id).count != 0
        return render json: { status: :booked, message: 'このレッスンはすでに予約されています' } if @book.user_id.present?
